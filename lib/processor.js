...
function start(server) {
  ...
  client.on('message', (receiveTopic, message) => {
    ...
    server.sendStreamEvent(event);

    if (event.payload.startsWith('ping')) {
      const command = {
        deviceId: event.deviceId,
        payload: 'pong' + event.payload.substring(event.payload.indexOf(':'))
      };
      lastResponse = command.payload;
      server.sendResponseEvent(lastResponse);
      server.sendCommandEvent(command);
